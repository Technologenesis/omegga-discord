...
cd omegga-discord
git checkout tags/v1.1.1 #optional; the master branch should usually be stable,
...
| enable-chat-log | Send chat logs to `chat-channel-id`? | false | `chat-channel-id`[*](#discordids).  Optionally, `log-game-events` can be used to log in-game events alongside chat messages. `server-event-whitelist` and `server-event-blacklist` are regexes that can be used to control which server events are relayed to Discord chat; by default they screen out everything but join/leave messages.  Removing the whitelist will only screen out autosave messages; removing both will allow all server events to come through.  Only events that match the whitelist AND don't match the blacklist are relayed. `compact-chat` enables a simpler, more compact format for chat logs. | Ensure that your bot is permitted to send messages to `chat-channel-id`
