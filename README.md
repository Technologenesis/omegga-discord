...
| enable-chat-log | Send chat logs to `chat-channel-id`? | false | `chat-channel-id`[*](#discordids).  Optionally, `log-game-events` can be used to log in-game events alongside chat messages. | Ensure that your bot is permitted to send messages to `chat-channel-id`
